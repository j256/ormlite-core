...
public class JdbcDatabaseAccessImplTest extends BaseOrmLiteTest {
	...
	public void testQueryForLong() throws Exception {
		DatabaseAccess databaseAccess = new JdbcDatabaseAccess(connectionSource);
		Dao<Foo, Object> dao = createDao(Foo.class, true);
		Foo foo = new Foo();
		long id = 21321321L;
		foo.id = id;
		assertEquals(1, dao.create(foo));
		assertEquals(id, databaseAccess.queryForLong("select id from foo"));
	}
	...
	public void testQueryForLongNoResult() throws Exception {
		...
		databaseAccess.queryForLong("select id from foo");
	}
	...
	public void testQueryKeyHolderNoKeys() throws Exception {
		...
		databaseAccess.insert("insert into foo (id) values (1)", new Object[0], new int[0], keyHolder);
	}
